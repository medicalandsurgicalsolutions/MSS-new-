...
const getAllOrders = async (req, res) => {
  ...
  try {
    ...
    const orders = await Order.find(queryObject)
      .select(
        "_id invoice paymentMethod subTotal total user_info discount shippingCost status deliveryDate isCancelByCustomer createdAt updatedAt tracklink trackno deliveryPartner"
      )
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(limits);
...
const updateOrderTrackLink = (req, res) => {
  const { status, tracklink } = req.body;
  ...
  if (tracklink) {
    updateFields.tracklink = tracklink;
  }
...
const updateOrderDeliveryPartner = (req, res) => {
...
const updateOrderTrackNo = async(req, res) => {
  const { status, trackno } = req.body;
  ...
  const order = await Order.findById(req.params.id);
  // console.log("Track Link ", order?.tracklink);
  const update = await Order.updateOne({ _id: req.params.id }, { $set: updateFields });
  if(update){
    sendTrackSms(order?.tracklink, req.body.trackno, order?.user_info?.contact);
    res.status(200).send({
      message: "Track No Updated Successfully!",
    });
  }
};

const sendTrackSms = async(tracklink, trackno, phone)=>{
  const respo = await axios.get(`${process.env.SMS_API_URL}?username=${process.env.SMS_USER_NAME}&msg_token=${process.env.SMS_MESSAGE_TOKEN}&sender_id=${process.env.SMS_SENDER_ID}&message=Your+order+is+on+its+way+Track+your+shipment+using+the+link+below%3A+${tracklink}+Docket+No%3A+${trackno}+Need+help+Contact+us+at+care%40medicalsurgical.org.+Thank+you+for+choosing+Medical+%26+Surgical+Solutions&mobile=${phone}`);
}
