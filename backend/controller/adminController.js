...
const { ADMIN_STAFF_ID } = require("../utils/admin");
...
const registerAdmin = async (req, res) => {
  try {
    const isAdded = await Staff.findOne({ email: req.body.email });
    if (isAdded) {
...
const loginAdmin = async (req, res) => {
  try {
    ...
    if (staff && bcrypt.compareSync(req.body.password, staff.password)) {
      const token = signInToken(staff);
      const roleAndPermissions = await getRoleWithPermissions(staff.role);
...
const forgetPassword = async (req, res) => {
  const isAdded = await Staff.findOne({ email: req.body.verifyEmail });
  if (!isAdded) {
    ...
    const body = {
      ...
      <p>A request has been received to change the password for your <strong>${process.env.APP_NAME}</strong> account </p>
        ...
        <p style="margin-top: 35px;">If you did not initiate this request, please contact us immediately at ${process.env.SUPPORT_EMAIL}</p>
...
const updateStaff = async (req, res) => {
  try {
    const admin = await Staff.findOne({ _id: req.params.id });
...
const updatedStatus = async (req, res) => {
  try {
    ...
    res.send({
      message: `Staff ${
        newStatus === "show" ? "Activated" : "Deactivated"
      } Successfully!`,
    });
  } catch (err) {
    res.status(500).send({
