...
const Checkout = () => {
  ...
  const savePrescription = (itemId, fileUrl) => {
    const prescriptions = JSON.parse(localStorage.getItem("prescriptions")) || {};
    prescriptions[itemId] = fileUrl;
  ...
  // Updated submitHandler to include prescription
  const submitHandlerWithPrescription = async (formData) => {
    try {
      if (isEmpty) return;
      ...
      const orderItems = items.map((item) => {
        ...
        return {
          ...item,
          prescriptionUrl: prescriptions[item.id] || "",
        };
      });
      ...
      const result = await createOrderRazorpay(orderData);

      if (result?.success) {
        notifySuccess("Order placed successfully");
        localStorage.removeItem("prescriptions"); // clear after order
      }
    } catch (err) {
        ...
        <div className="py-10 lg:py-12 px-0 2xl:max-w-screen-2xl w-full xl:max-w-screen-xl flex flex-col md:flex-row lg:flex-row">
          <div className="md:w-full lg:w-3/5 flex h-full flex-col order-2 sm:order-1 lg:order-1">
            <div className="mt-5 md:mt-0 md:col-span-2">
              <form onSubmit={handleSubmit(submitHandlerWithPrescription)}>
                {hasShippingAddress && (
                        ...
                        {showingTranslateValue(storeCustomizationSetting?.checkout?.shipping_two_desc) && (
                          <div className="col-span-6 sm:col-span-3">
                            <InputShipping
                              ...
                              cost={Number(storeCustomizationSetting?.checkout?.shipping_two_cost) || 0}
                            />
                            <Error errorName={errors.shippingOption} />
                          </div>
                        )}
          ...
          <div className="md:w-full lg:w-2/5 lg:ml-10 xl:ml-14 md:ml-6 flex flex-col h-full md:sticky lg:sticky top-28 md:order-2 lg:order-2">
            <div className="border p-5 lg:px-8 lg:py-8 rounded-lg bg-white order-1 sm:order-2">
              ...
              <div className="overflow-y-scroll flex-grow scrollbar-hide w-full max-h-64 bg-gray-50 block">
                {items.map((item) => (
                  <CartItem key={item.id} item={item} currency={currency} savePrescription={savePrescription} />
                ))}
                {isEmpty && (
              ...
              {/* Coupon, Subtotal, Shipping, COD, Discount, Total (unchanged) */}
              {/* ...existing summary code remains here */}
            </div>
          </div>
        </div>
      </div>
