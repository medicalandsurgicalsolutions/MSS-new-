  ...
  return (
    ...
    >
      ...
      {/* 🌟 Contact Section with Google Map */}
        ...
        {/* Left - Contact Form */} 
          <div> 
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-3"> Get in <span className="text-cyan-600">Touch</span> </h2> 
          <p className="text-gray-600 mb-8 text-base"> We'd love to hear from you! Whether you’re curious about our products or need support, we’re here to help. </p>
          ...
          <div>
          <InputArea register={register} name="name" label="Name" placeholder="Enter your name" /> <Error errorName={errors.name} />
          </div> 
          <div> <InputArea register={register} name="email" label="Email" type="email" placeholder="Enter your email" /> <Error errorName={errors.email} />
          </div> 
          </div> 
          <div className="grid md:grid-cols-2 gap-5">
          <div>
          <InputArea register={register} name="number" label="Phone" type="number" placeholder="Enter number" /> <Error errorName={errors.number} />
          </div> 
          <div> 
          <InputArea register={register} name="subject" label="Subject" placeholder="Enter subject" />
          <Error errorName={errors.subject} /> </div> 
          </div>
          <div>
          <Label label="Message" /> <textarea {...register("message", { required: "Message is required!" })} 
            rows="4" placeholder="Type your message..." className="w-full px-4 py-3 rounded-lg border border-gray-300 bg-gray-50 
              focus:bg-white focus:border-cyan-500 transition-all outline-none" >
              </textarea> <Error errorName={errors.message} /> 
              </div> 
              <button type="submit" disabled={mailLoading} className="w-full py-3 rounded-lg bg-gradient-to-r from-cyan-500 to-cyan-600 text-white
                    font-semibold shadow hover:shadow-cyan-300/60 transition-all" > 
