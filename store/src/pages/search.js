...
// internal imports import Layout from "@layout/Layout"; 
...
const Search = ({ products, attributes, brands, categories, departments, nextPage, prevPage, totalProduct, }) => {
  const { t } = useTranslation(); const { showingTranslateValue } = useUtilsFunction(); 
   const { isLoading, setIsLoading } = useContext(SidebarContext); const [visibleProduct, setVisibleProduct] = useState(4000);
      ...
      if (apply) {
        ...
        if (category.length > 0) { 
          services = services?.filter((service) => { 
            ...
            ${service.category._id},isCategoryMatched: 
            ${isCategoryMatched}, isCategoriesMatched:
            ${isCategoriesMatched});
  ...
  const handleAttributeChange = (body) => { 
    ...
    const exists = attribute.some( 
      (attr) => attr.attributeId === attributeId && attr.variantId === 
        variantId 
    );
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (brandRef.current && !brandRef.current.contains(event.target)) { setIsOpenBrand(false); 
         }
