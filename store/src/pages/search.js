...
const Search = ({
  ...
  const { query, category: queryCategory, brand: queryBrand, title, slug, _id } =
    searchQuery || {};
  ...
  useEffect(() => {
    ...
    if (savedScrollData) {
      try {
        ...
        // Check if the scroll data is still valid (within expiry)
  ...
  // Ensure filteredProducts follows incoming products
  useEffect(() => {
  ...
  // Reset filters when route path changes
  useEffect(() => {
    reset();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [asPath]);

  // Apply filters when "apply" toggles true
  useEffect(() => {
    if (!apply) return;

    let services = Array.isArray(products) ? [...products] : [];
    ...
    if (category.length > 0) {
      services = services?.filter((service) => {
        ...
        const isCategoriesMatched = Array.isArray(service?.categories)
          ? service.categories?.some((catId) => category?.includes(catId?._id))
          : false;

        // Debug-safe log (commented)
        // console.log(
        //   `Service ID: ${service?.category?._id}, isCategoryMatched: ${isCategoryMatched}, isCategoriesMatched: ${isCategoriesMatched}`
        // );
    ...
    // Filter by attributes if any selected
    if (attribute.length > 0) {
      services = services?.filter((service) => {
        const variantAttributes = Array.isArray(service?.variants)
  ...
  return isLoading ? (
    ...
    <Layout title="Search" description="This is search page">
      <div className="mx-auto max-w-screen-2xl px-3 sm:px-10">
        {/* Optional top cards / carousel - left as comments in original */}
        ...
        <div className="flex py-10 lg:py-12">
          <div className="w-full lg:flex">
            ...
            >
              ...
              {/* Uncomment and use if needed (kept logic but commented as in original)
              {attributes.map((a, j) => (
                    ...
                    {a.variants.map((v, i) => (
                      <button
                        ...
                        className={`px-3 py-1 text-xs rounded-full ${
                          attribute.some(
                            (attr) => attr.attributeId === a._id && attr.variantId === v._id
                          )
...
export const getServerSideProps = async (context) => {
  ...
  const [data, attributes, brands, categories, departments] =
    await Promise.all([
