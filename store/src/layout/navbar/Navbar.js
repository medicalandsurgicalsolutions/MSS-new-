        ...
        {/* Logo */} 
          <Link href="/" className="mr-3 lg:mr-12 xl:mr-12 hidden md:hidden lg:block" > 
            <div className="relative w-32 h-16">
              <Image src={storeCustomizationSetting?.navbar?.logo || logo} alt="Company Logo" fill className="object-contain" priority /> 
                </div>
                </Link> 
               ...
               {filteredProducts?.length > 0 && ( <ul ref={dropdownRef} 
                 className="absolute w-full bg-white shadow-md rounded-md mt-12 max-h-60 overflow-y-auto z-20" > 
                 {filteredProducts.map((item, index) => ( <li key={index} onClick={() => handleSuggestionClick(item?.slug)} 
            ...
            {/* Phone Info Section */} 
              ...
              {/* Right Section (Cart + User) */} 
                <div className="hidden md:hidden lg:flex items-center"> 
                  <button aria-label="Total" onClick={toggleCartDrawer} 
                  className="relative px-5 text-white text-2xl" > 
                    <span className="absolute top-0 right-0 inline-flex items-center justify-center p-1 h-5 w-5 text-xs 
                        font-medium text-red-100 transform -translate-x-1/2 -translate-y-1/2 bg-red-500 rounded-full"> 
                            {totalItems} 
                      </span> 
                    <FiShoppingCart className="w-6 h-6 drop-shadow-xl" />
                      </button> 
                    <button className="pl-5 text-white text-2xl" aria-label="Login">
                      {userInfo?.image ? ( <Link href="/user/dashboard" className="relative w-6 h-6">
                        <Image width={29} height={29} src={userInfo?.image} alt="user" className="bg-white rounded-full" /> 
                        </Link> ) : userInfo?.phone ? ( <Link href="/user/dashboard"> 
                          <FiUser className="w-6 h-6 drop-shadow-xl" /> </Link> ) : ( <Link href="/auth/login">
                            <FiUser className="w-6 h-6 drop-shadow-xl" /> </Link> )}
                          </button> 
                        </div> 
                      </div> 
                    </div>
