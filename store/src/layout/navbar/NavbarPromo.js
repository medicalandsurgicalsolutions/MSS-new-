  ...
  return (
    <>
      <div className="hidden lg:block xl:block bg-gray-100 border-b text-sm text-black">
        <div className="max-w-screen-2xl mx-auto px-8 sm:px-10 lg:px-16 flex justify-between items-center">
          
          {/* Home */}
          <Link onClick={() => setIsLoading(!isLoading)} href="/" className=" mx-4 py-2 hover:text-emerald-600" >
            Home
          </Link>

          {/* Quick Delivery Subtitle */}
          ...
          {data[0]?.children?.slice(0, 6)?.map((category, index) => (
            <div
              key={index}
              className="relative cursor-pointer group py-2 flex items-center hover:text-emerald-600 space-x-1"
              onClick={() =>
                handleSubCategory(
                  category?._id,
        ...
        <div> {storeCustomizationSetting?.home?.quick_delivery_subtitle?.en && ( 
          <>
          <Link onClick={() => setIsLoading(!isLoading)} href="/search?query=latest" >
          <div className="mx-4 py-2 hover:text-emerald-600"> { storeCustomizationSetting?.home?.quick_delivery_subtitle ?.en } </div> 
              ...
              {data[0]?.children?.slice(0, 6)?.map((category, index) => { return ( 
                <div key={index} className="relative cursor-pointer group py-2" onClick={() => handleSubCategory( category?._id, showingTranslateValue(category?.name) ) } > 
                    <div className=" mx-4 group hover:text-emerald-600 flex items-center space-x-2"> <div>
        ...
        {category?.children && ( 
          <div className="absolute left-0 w-auto top-full rounded-md hidden group-hover:block bg-cyan-500 text-white shadow-lg p-4 gap-y-2 gap-x-6 " style={{ display: "grid", gridTemplateColumns: repeat(${Math.ceil( category.children.length / 8 )}, auto), }} >
              {category?.children?.map((subCategory, subIndex) => (
                <div className="border-b "> 
                {/* <div className="h-8 w-8"> 
                <Image src={subCategory?.icon} alr="sub category" height={300} width={300} /> </div> */} 
                <div key={subIndex} className="block px-1 text-sm cursor-pointer py-1 hover:translate-x-1.5 duration-100 whitespace-nowrap"
                  onClick={(event) => { event.stopPropagation(); handleSubNestedCategory( subCategory?._id, showingTranslateValue(subCategory?.name) ); }
                    }> 
                  {subCategory?.name?.en} </div>
                  </div> )
                  )} 
                  </div> )} 
                  </div> )} 
