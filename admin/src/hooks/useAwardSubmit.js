...
const useAwardSubmit = (id) => {
  ...
  const onSubmit = async ({ name, description, website }) => {
    ...
    //   console.log("Client Log ", clientData);
      ...
      if (id) {
        const res = await AwardServices.updateAward(id, clientData);
        setIsUpdate(true);
        ...
        const res = await AwardServices.addAward(clientData);
        setIsUpdate(true);
        setIsSubmitting(false);
        notifySuccess(res.message);
        closeDrawer();
  ...
  useEffect(() => {
    ...
    if (id) {
      (async () => {
        try {
          const res = await AwardServices.getAwardById(id);
