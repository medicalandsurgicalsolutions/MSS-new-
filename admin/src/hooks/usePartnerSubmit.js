...
const usePartnerSubmit = (id) => {
  ...
  const onSubmit = async ({ name, description, website }) => {
    try {
      ...
      if (id) {
        const res = await PartnerServices.updatePartner(id, clientData);
        setIsUpdate(true);
        ...
        const res = await PartnerServices.addPartner(clientData);
        setIsUpdate(true);
        setIsSubmitting(false);
        notifySuccess(res.message);
        closeDrawer();
  ...
  useEffect(() => {
    ...
    if (id) {
      (async () => {
        try {
          const res = await PartnerServices.getPartnerById(id);
